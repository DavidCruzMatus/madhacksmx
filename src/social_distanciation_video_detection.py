...
while True:	
	...
	if not frame_exists:
		...
		for point in transformed_downoids:
			...
			LatN= Lat0 - (y*(Lat0-Lat1))/Tamy
			LongN= Long0 + (x*(Long1-Long0))/Tamx
			...
			for i,pair in enumerate(itertools.combinations(transformed_downoids, r=2)):
				...
				if math.sqrt( (pair[0][0] - pair[1][0])**2 + (pair[0][1] - pair[1][1])**2 ) < int(distance_minimum):
					...
					if not (pair[0][0] > width or pair[0][0] < 0 or pair[0][1] > height+200  or pair[0][1] < 0 or pair[1][0] > width or pair[1][0] < 0 or pair[1][1] > height+200  or pair[1][1] < 0):
						...
						bx= pair[1][0]
						by= pair[1][1]
						LatNa= Lat0 - (ay*(Lat0-Lat1))/Tamy
						LongNa= Long0 + (ax*(Long1-Long0))/Tamx
						LatNb= Lat0 - (by*(Lat0-Lat1))/Tamy
						LongNb= Long0 + (bx*(Long1-Long0))/Tamx
						print("a",LatNa,LongNa)
						print("b",LatNb,LongNb)
						cursor = connection.cursor()
						dataToInsert=[(cuenta,LatNa,LongNa,1,today), (cuenta,LatNb,LongNb,1,today)]
						cursor.execute("UPDATE MLUSER.COVID_SOCIAL_DISTANCIA_T SET DISTANCIA=1 WHERE PERSONA="+str(cuenta)+" AND LATITUD="+str(LatNa)+" AND LONGITUD="+str(LongNa))
						cursor.execute("UPDATE MLUSER.COVID_SOCIAL_DISTANCIA_T SET DISTANCIA=1 WHERE PERSONA="+str(cuenta)+" AND LATITUD="+str(LatNb)+" AND LONGITUD="+str(LongNb))
						connection.commit()
